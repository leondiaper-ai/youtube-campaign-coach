...
/* ── A SECOND PUBLISHING SURFACE ──────────────────────────────────────
   Campaigns that run across two channels are merged by
   /live/second-surface.js, which the Intelligence Hub loads too — so the
   deck and the Hub cannot disagree about a campaign's totals, because
   they are running the same merge over the same payload. A host that has
   not loaded it simply shows the artist channel's campaign. */
...
async function loadLiveCampaign(){
  ...
  try {
    if (window.CampaignSecondSurface) await window.CampaignSecondSurface.merge(d);
