...
function assetStrip(rest, total, shownAbove){
  ...
  return `<div class="strip stagger">
    ${rest.map(a => `<a class="sti${a.sourceAspect === 'portrait' ? ' sti-p' : ''}"
      ...
      title="${esc(a.dateLabel)} · ${esc(a.formatLabel)}${
        a.source ? ' · ' + esc(a.source) : ''} · ${esc(a.title)}">
...
/* ═══════════════════════════════════════════════════════════════════
   A SECOND PUBLISHING SURFACE

   Some campaigns do not run entirely on the artist's own channel. Palaye
   Royale is the case that forced this: the official videos for Feel
   Something, Great. and Sad Generation are on Sumerian Records, so a
   campaign page reading only the artist channel would be missing the
   hero. The campaign is the sum of its assets, wherever they were
   published, and this merges the second surface into that sum.

   ── WHAT IT WILL NOT DO ────────────────────────────────────────────
   It never brings the second channel's OWN performance across. Sumerian
   has 3,664 uploads and almost none of them are Palaye; pulling a label's
   channel totals into an artist's campaign would be the most misleading
   thing this layer could do. Only uploads the label-watch endpoint has
   confidently tied to the artist — its artist's name in the video TITLE —
   are merged. Its `flagged` list, which exists so a human can confirm
   the uncertain ones, is not read here and never reaches the page.

   ── AND WHY IT IS NOT A HEADLINE ───────────────────────────────────
   Once merged, an asset is just a campaign asset. Each one names where it
   was published, in the same small metadata line that already carried its
   date and format, and one sentence at the foot of the column says the
   campaign spans both. There is deliberately no per-channel statistic,
   no comparison and no panel: which channel a video sits on is a fact
   about the campaign, not the subject of the page.

   Decks that do not configure `alsoWatch` never execute a line of this.
   ═══════════════════════════════════════════════════════════════════ */

/* A matched label upload, in the shape the cover's asset renderers
   already speak. Aspect is derived from duration for the same reason the
   cover does it elsewhere: a vertical asset framed 16:9 puts YouTube's
   baked-in pillarbox back into the picture. */
...
/* ── A DATE THE LABEL PUBLISHED ITSELF ────────────────────────────────
   Release dates reach this page one of two ways: from the campaign plan,
   which is what a person entered, or from the campaign's own posts, which
   is what the world has been told. The second is evidence and worth
   surfacing — Sumerian's announcement carries "out Oct. 2nd!" in its
   title, which is a published date where the plan holds none.

   It is read only out of a CONFIDENTLY matched post, it is labelled with
   the channel that said it, and if the pattern is not there nothing is
   added. No date is ever inferred from a description, a thumbnail or a
   guess at a release cycle. */
function momentFromPost(m, sourceName){
  ...
  /* Built in UTC, deliberately. `new Date('Oct 2, 2026')` is LOCAL
     midnight, and every date label on this page is read in UTC — so on a
     machine east of Greenwich the two disagree and a 2 October release
     prints as 1 October. A date the label published is worth getting
     right to the day. */
  ...
  /* A date that has already passed is history, not what's next. */
  if (when.getTime() < Date.now() - 864e5) return null;
  /* The single's name, where the post sets it in quotes. Falls back to
     the plain word rather than printing a punctuated fragment. */
  const q = (m.title || '').match(/['"\u2018\u2019\u201C\u201D]([^'"\u2018\u2019\u201C\u201D]{2,60})['"\u2018\u2019\u201C\u201D]/);
  return {
    kind: 'NEXT',
...
async function mergeSecondSurface(d){
  const cfg = CFG.alsoWatch;
  if (!cfg || !cfg.label || !cfg.artist) return;

  const start = d.metrics && d.metrics.campaignStart && d.metrics.campaignStart.at;
  const api = cfg.api || '/api/label-watch';
  ...
  /* Every asset on the page now names its channel, including the
     artist's own. Labelling only the imported ones would read as though
     the label uploads were the exception rather than half the campaign. */
  ...
  if (imported.length) {
    ...
    /* The server counted the artist channel's uploads and said so in
       ...
       server's own wording and its careful refusal to call this a
       confirmed campaign start exactly as written. */
    ...
    if (tl && Array.isArray(tl.moments)) {
      ...
      for (const m2 of (w.matched || [])) {
        const mo = momentFromPost(m2, sourceName);
  ...
  /* One line, at the foot of the asset column. */
  d.assetNote = cfg.note
    || `Campaign activity includes ${artistName}'s artist channel and `
  ...
  <div class="inner">
    ...
    <div class="covergrid">
      <div>
        ...
        ${d.assetNote ? `<p class="assetnote reveal">${esc(d.assetNote)}</p>` : ''}
      </div>
...
function switchTab(name){
  ...
  /* A deck with no Ideas pane is a valid host — it just has one tab. The
     campaign view is always the fallback, so a stray call cannot leave a
     reader looking at a hidden deck. */
  const isIdeas = name === 'ideas' && !!ideas;
  if (deck) deck.style.display = isIdeas ? 'none' : '';
  if (ideas) ideas.style.display = isIdeas ? '' : 'none';
...
async function loadLiveCampaign(){
  ...
  /* A campaign that publishes on more than one channel, merged before
     anything renders so the cover is built from the whole campaign
     rather than patched afterwards. A failure here must not cost the
     page the cover it already has the data for. */
  try { await mergeSecondSurface(d); }
  catch (e) { console.warn('[live] second surface unavailable —', e && e.message); }
  ...
  /* The playbook is optional. A deck with no Ideas pane, or a campaign
     whose rollout has no items yet, gets the campaign home alone — and
     showing a tab bar whose second tab is empty is worse than showing
     none. */
  const ideas = document.getElementById('ideas');
  if (ideas && (d.rollout && (d.rollout.items || []).length)) {
    ideas.innerHTML = buildIdeas(d);
